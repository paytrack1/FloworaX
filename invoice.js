...
<body>
  ...
  <div style="border-top:1px solid #E2E8F0;padding-top:16px;text-align:center;">
    <p style="font-size:11px;color:#CBD5E1;">Generated by Paytrack · Thank you for your business</p>
  </div>
</body>
</html>`;
};
  ...
  const handleCreate = async () => {
    ...
    try {
      const res  = await fetch(`${BACKEND_URL}/api/invoices`, {
        ...
        body: JSON.stringify({ ...form, total: subtotal }),
      });
      const data = await res.json();
      if (data.success) {
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center"
          ...
          <div className="bg-white rounded-t-3xl w-full max-w-lg p-6 shadow-2xl"
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setPreview(null)} className="text-[#94A3B8] text-xl font-bold">✕</button>
            </div>
              ...
              {(preview.items || []).map((item, i) => (
                <div key={i} className="flex justify-between py-2 text-sm border-b border-[#F8FAFC]">
                  <div>
                    ...
                    <p className="text-[#94A3B8] text-xs">{item.quantity} × {formatNaira(item.unitPrice)}</p>
                  </div>
            ...
            <div className="flex justify-between items-center bg-[#EEF4FF] rounded-xl px-4 py-3 mb-5">
              ...
              <span className="font-black text-[#185FA5] text-lg">{formatNaira(preview.total)}</span>
            </div>
                ...
                <div key={invoice._id} className="bg-white rounded-2xl border border-[#E2E8F0] p-4 shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-[#94A3B8] text-xs mt-0.5">{invoice.invoiceNumber} · {formatDate(invoice.issueDate)}</p>
                    </div>
                  ...
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <p className="font-black text-[#185FA5] text-base">{formatNaira(invoice.total)}</p>
                  </div>
