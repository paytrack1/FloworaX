    ...
    <aside className="hidden lg:flex fixed top-0 left-0 bottom-0 w-64 bg-[#0F172A] flex-col z-50">
      <div className="px-6 py-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-black text-lg leading-none tracking-tight">Flowora</p>
            <p className="text-[#2F5FB3] font-black text-sm leading-none">Lite</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 mb-4">
        <p className="text-slate-600 text-[10px] font-black uppercase tracking-widest">Navigation</p>
      </div>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-left ${
                isActive ? 'bg-[#2F5FB3] text-white' : 'text-slate-400 hover:bg-white/5 hover:text-white'
              }`}
            >
      ...
      <div className="p-4 border-t border-white/5">
        <div className="flex items-center gap-3 px-2 py-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white font-bold text-sm truncate leading-none mb-0.5">{user?.businessName || 'My Store'}</p>
            <p className="text-slate-500 text-[10px] font-bold truncate">{user?.email || ''}</p>
          </div>
          <button onClick={logout} className="p-1.5 text-slate-500 hover:text-red-400 transition-colors flex-shrink-0" title="Log out">
            <LogOut size={16} />
          </button>
        </div>
      </div>
