...
const PLAN_CATALOG = {
  free: {
    ...
    limits: { monthlySales: 50, monthlyBookings: 40, services: 8, events: 3 },
  ...
  pro: {
    ...
    limits: { monthlySales: 500, monthlyBookings: 200, services: 20, events: null },
    badge: 'Most popular',
  },
  business: {
    name: 'Business',
    ...
    limits: { monthlySales: null, monthlyBookings: null, services: null, events: null },
    badge: 'Enterprise',
  },
};
...
'@ | Set-Content -Path "src\middleware\plan.js" -Encoding UTF8

Write-Host "plan.js written. Verifying..." -ForegroundColor Cyan
node -c "src\middleware\plan.js"
if ($LASTEXITCODE -eq 0) { Write-Host "plan.js OK" -ForegroundColor Green } else { Write-Host "plan.js FAILED" -ForegroundColor Red }